...
function objectMerge(array, tiempo) {
  return new Promise((resolve, reject) => {
    const seconds = tiempo();
    if (seconds && typeof seconds === 'number' && seconds > 0) {
      setTimeout(() => {
        if (Array.isArray(array()) && array().length >= 1) {
          resolve(array().reduce((object, arrayObject) => ({ ...object, ...arrayObject })));
        } else {
...
/*
La funcion contador debe retornar una funcion que cuando sea invocada retorne un valor
creciente.
...
de la invocacion.
Cuando la funcion que retornaste sea invocada de nuevo con un argumento con el cual se habia
invocado anterioremente, no deberia invocar de nuevo a cb deberia retornar el resultado (previamente guardado)
